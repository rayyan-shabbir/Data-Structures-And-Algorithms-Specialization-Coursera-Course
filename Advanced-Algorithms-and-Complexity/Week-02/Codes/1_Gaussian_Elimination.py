...
# Class to represent a vector with basic vector operations
class Vector(object):
...
# Class to represent a linear system of equations
...
# Class to handle decimal operations with a tolerance for near-zero values
...
# Function to read the linear system of equations from input
def ReadEquation():
    size = int(input())
    if size == 0:
        sys.exit()
...
# Function to print a column of values with a specified precision
def PrintColumn(column):
    size = len(column)
    if size == 0:
        sys.exit()
    for row in range(size):
        print("%.4lf" % column[row], end=' ')
    print()
 
 
# Main function to read the system, solve it, and print the result
